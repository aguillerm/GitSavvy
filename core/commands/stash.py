...
class GsApplyStashCommand(WindowCommand, GitCommand):
    ...
    def run(self, stash_id=None):
        if stash_id is None:
            show_stash_panel(self.get_stashes(), self.do_apply)
        else:
...
class GsPopStashCommand(WindowCommand, GitCommand):
    ...
    def run(self, stash_id=None):
        if stash_id is None:
            show_stash_panel(self.get_stashes(), self.do_pop)
        else:
...
class GsShowStashCommand(WindowCommand, GitCommand):
    ...
    def run(self, stash_ids=[]):
        if len(stash_ids) == 0:
            show_stash_panel(self.get_stashes(), self.do_show)
        else:
...
class GsDiscardStashCommand(WindowCommand, GitCommand):
    ...
    def run(self, stash_id=None):
        if stash_id is None:
            show_stash_panel(self.get_stashes(), self.do_drop)
        else:
